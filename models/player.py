...
from base import intpk, intfk_gid, intfk_tid, intfk_pid
...
class Player(Base):
    ...
    pos: Mapped[str] # Player position
...
class SkaterStatAdvanced(Base):
    ...
    cfp: Mapped[float] # Corsi For Percentage (percentage of Corsi For Events vs. opponent while one ice)
    crel: Mapped[float] # Relative Corsi For Percentage for player's team when that player is on-ice vs. when not
